...
public class FileTailingService {
    ...
    private final ConcurrentHashMap<String, ApacheCommonsFileTailingMessageProducer> producers = new ConcurrentHashMap<>();
    ...
    public void stopTailing(String filePath) {
        ApacheCommonsFileTailingMessageProducer producer = producers.remove(filePath);
        if (producer != null) {
            producer.stop();
        }
    }
